...
class MesaRetiroInactivateUserLicensePromptWizard(models.TransientModel):
    _name = 'mesa.retiro.inactivate.user.license.prompt.wizard'
    _description = '¿Retirar licencias del usuario? (inactivar usuario)'
    ...
    @api.depends('retiro_wizard_id', 'retiro_wizard_id.contact_id')
    def _compute_license_count(self):
        for wiz in self:
            parent = wiz._parent_retiro_wizard()
            if not parent or not parent.contact_id:
                wiz.license_count = 0
                continue
            lines = parent._discover_user_license_lines(
                parent.partner_id,
                parent.contact_id,
            )
            wiz.license_count = len(lines)

    def action_yes_remove_licenses(self):
        self.ensure_one()
    ...
    def action_no_remove_licenses(self):
        self.ensure_one()
        parent = self._parent_retiro_wizard()
        if not parent:
            raise UserError(_('No se encontró el asistente de retiro.'))
